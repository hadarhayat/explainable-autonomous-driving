    ...
    with open(input_jsonl, 'r') as infile, open(output_csv, 'w', newline='') as outfile:
        ...
        for i, line in enumerate(infile):
            ...
            *** EXAMPLE OF REQUIRED RESPONSE FORMAT ***
            User: The vehicle is currently at position (239.3, -169.3, 0.2), it is facing east with a yaw of 0.4 degrees.
            The orientation is level, with a pitch of -0.0 and roll of 0.0 degrees.
            The current speed is 0.0 km/h heading east.

            - GPS Route Instruction: Turn Left
            - Previous Action: Throttle: 0.0, Steer: 0.0, Brake: 1.0

            There is a Red traffic light coming up ahead.
            A vehicle is approaching from the front, 35.19 meters away, traveling at 0.0 km/h.

            What should the vehicle do next?
            
            Assistant: Thought: I am at or approaching a red traffic light, so I am braking to stop safely. There is a vehicle in my path, requiring me to brake or yield. I am keeping the vehicle stopped.
            High-Level Action: Hard Brake / Emergency Stop
            Low-Level Control: Throttle: 0.0, Steer: 0.0, Brake: 1.0
            *** END EXAMPLE. Now answer the actual prompt using this exact format. ***
            """
            ...
            strict_system_prompt = system_prompt + one_shot_example
            ...
            llm_raw_response = send_prompt_to_llm(strict_system_prompt, user_prompt)
...
if __name__ == "__main__":
    run_evaluation("data/captures/testing/testing_dataset.jsonl", "results/thesis_evaluation_oneshot_llama.csv")